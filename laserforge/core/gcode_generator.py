...
class GCodeGenerator:
    ...
    def _laser_off_cmd(self) -> str:
        mode = self.settings.laser_mode
        if mode == "M106":
            return "M107          ; Ensure laser is OFF"
        return "M5"
    ...
    def generate_job(self, entities: List[LaserEntity]) -> GCodeJobResult:
        ...
        if self.settings.enable_z_moves:
            gcode_lines.append(f"G0 Z0 F{self.settings.rapid_speed:.0f}")
        ...
        for lid in sorted(layer_groups.keys()):
            ...
            gcode_lines.append(f"; --- Layer {layer.name} ({layer.color}) Mode: {layer.mode} ---")
            if layer.air_assist:
                gcode_lines.append(self.settings.air_assist_cmd)
            ...
            for pass_idx in range(passes):
                if passes > 1:
                    ...
                    if pass_idx > 0 and getattr(layer, "pass_delay_sec", 0.0) > 0.0:
                        ...
                        gcode_lines.append("M5 ; 3W Diode Cooldown")
                ...
                for img_ent in image_entities:
                    ...
                    try:
                        ...
                        fire_cmd = f"G4 P{fire_dwell / 1000.0:.3f}" if fire_dwell > 0.0 else ""
                        off_dwell = self.settings.laser_off_delay_ms
                        off_cmd = f"G4 P{off_dwell / 1000.0:.3f}" if off_dwell > 0.0 else ""
                            ...
                            for line in scanlines:
                                ...
                                for cluster in clusters:
                                    ...
                                    if inline_s and not continuous_streaming:
                                        gcode_lines.append(laser_cmd)

                                    # 2. Lead-in acceleration with laser OFF
                                    current_raster_feed = None
                                    ...
                                    for seg_idx, (seg_x1, seg_x2, p_ratio, _) in enumerate(cluster):
                                        ...
                                        if inter_gap > 0.01:
                                            ...
                                            if inline_s:
                                                if current_raster_feed != feed:
                                                    gcode_lines.append(f"G1 X{seg_x1:.3f} S0 F{feed:.0f}")
                                                    current_raster_feed = feed
                                                else:
                                        ...
                                        if inline_s:
                                            if current_raster_feed != feed:
                                                gcode_lines.append(f"G1 X{seg_x2:.3f} S{seg_p} F{feed:.0f}")
                                                current_raster_feed = feed
                                            else:
                    ...
                    for poly, meta in zip(optimized_fill_paths, optimized_fill_meta):
                        ...
                        for seg_x1, seg_x2, seg_y in fill_segs:
                            ...
                            if continuous_streaming:
                                ...
                                fire_dwell = self.settings.laser_fire_delay_ms
                                if fire_dwell > 0.0:
                                    gcode_lines.append(f"G4 P{fire_dwell / 1000.0:.3f}")
                                if eff_feed != current_cut_feed:
                                ...
                                off_dwell = self.settings.laser_off_delay_ms
                                if off_dwell > 0.0:
                                    gcode_lines.append(f"G4 P{off_dwell / 1000.0:.3f}")
            ...
            if layer.air_assist:
                ...
                gcode_lines.append(self.settings.air_assist_off_cmd)

        # Footer
        gcode_lines.append("; --- Job Finished ---")
        gcode_lines.append(self._laser_off_cmd())
        ...
        if finish_mode == "Job Start":
            gcode_lines.append(f"G0 X{job_start_x:.3f} Y{job_start_y:.3f} F{self.settings.rapid_speed:.0f}")
        elif finish_mode in ("Park Position", "Park"):
            park_x = self.settings.park_x
            park_y = self.settings.park_y
            gcode_lines.append(f"G0 X{park_x:.3f} Y{park_y:.3f} F{self.settings.rapid_speed:.0f}")
        elif finish_mode == "Hold Current":
            gcode_lines.append("; Hold Current Position")
        else:  # "Origin" or default
            gcode_lines.append(f"G0 X0 Y0 F{self.settings.rapid_speed:.0f}")
    ...
    def generate_contour_framing_gcode(self, target: Any) -> str:
        ...
        lines.append(self._laser_off_cmd())
        return "\n".join(lines)
    ...
    def generate_target_point_gcode(self, target_x: float, target_y: float, power_pct: float = 0.5) -> str:
        ...
        target_on_cmd = ('M106 S' + str(s_val)) if self.settings.laser_mode == 'M106' else ('M3 S' + str(s_val))
        lines = [
    ...
    def generate_burn_perimeter_gcode(
        ...
        for p_idx in range(max(1, passes)):
            if p_idx > 0 and pass_delay > 0:
                lines.append(f"G4 P{pass_delay:.2f}")
